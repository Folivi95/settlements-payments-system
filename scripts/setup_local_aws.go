package main

import (
	"context"
	"strings"
	"sync"
	"time"

	"github.com/aws/aws-sdk-go/aws"
	"github.com/aws/aws-sdk-go/aws/session"
	"github.com/aws/aws-sdk-go/service/s3"
	"github.com/aws/aws-sdk-go/service/sqs"
	zapctx "github.com/saltpay/go-zap-ctx"
	"go.uber.org/zap"

	cfg "github.com/saltpay/settlements-payments-system/cmd/config"

	awsAdapters "github.com/saltpay/settlements-payments-system/internal/adapters/aws"
	sqsAdapters "github.com/saltpay/settlements-payments-system/internal/adapters/aws/sqs"
	"github.com/saltpay/settlements-payments-system/internal/adapters/networking"
)

func main() {
	ctx := context.Background()

	config, err := cfg.LoadConfig(ctx)
	if err != nil {
		zapctx.Fatal(ctx, "Error initialising the app config", zap.Error(err))
	}

	err = networking.WaitForConnectivity(ctx, config.NetworkingCheckAddress, 60, time.Second)
	if err != nil {
		zapctx.Fatal(ctx, "failed to wait for connectivity", zap.Error(err))
	}

	awsSession, err := awsAdapters.CreateAwsSession(true, config.AwsRegion, config.AwsEndpoint)
	if err != nil {
		zapctx.Fatal(ctx, "failed to create AWS session", zap.Error(err))
	}

	connectedToLocalStack := time.Now()
	defer func() {
		zapctx.Info(ctx, "localstack components ready", zap.Duration("elapsed", time.Since(connectedToLocalStack)))
	}()

	var wg sync.WaitGroup
	wg.Add(12)

	// create sqs queues
	go createSqsQueue(ctx, &wg, awsSession, config.SqsUfxFileNotificationQueueName)
	go createSqsQueue(ctx, &wg, awsSession, config.SqsUfxFileNotificationDLQName)
	go createSqsQueue(ctx, &wg, awsSession, config.SqsBankingCircleUnprocessedQueueName)
	go createSqsQueue(ctx, &wg, awsSession, config.SqsBankingCircleUnprocessedDLQName)
	go createSqsQueue(ctx, &wg, awsSession, config.SqsIslandsbankiUnprocessedQueueName)
	go createSqsQueue(ctx, &wg, awsSession, config.SqsIslandsbankiUnprocessedDLQName)
	go createSqsQueue(ctx, &wg, awsSession, config.SqsBankingCircleProcessedQueueName)
	go createSqsQueue(ctx, &wg, awsSession, config.SqsBankingCircleProcessedDLQName)
	go createSqsQueue(ctx, &wg, awsSession, config.SqsBankingCircleUncheckedQueueName)
	go createSqsQueue(ctx, &wg, awsSession, config.SqsBankingCircleUncheckedDLQName)
	// create s3 bucket
	go createS3Bucket(ctx, &wg, awsSession, config.S3UfxPaymentFilesBucketName)
	// set up bucket notification
	go setUpS3BucketNotifications(ctx, &wg, awsSession, config.S3UfxPaymentFilesBucketName, config.SqsUfxFileNotificationQueueName)

	wg.Wait()
}

func createSqsQueue(ctx context.Context, wg *sync.WaitGroup, awsSession *session.Session, queueName string) {
	defer wg.Done()
	zapctx.Info(ctx, "creating sqs queue", zap.String("queue_name", queueName))

	svc := sqs.New(awsSession)
	_, err := svc.CreateQueue(&sqs.CreateQueueInput{
		QueueName: aws.String(queueName),
		Attributes: map[string]*string{
			"MessageRetentionPeriod": aws.String("86400"),
		},
	})
	if err != nil {
		zapctx.Fatal(ctx, "failed to create sqs queue", zap.String("queue_name", queueName), zap.Error(err))
	}

	err = sqsAdapters.WaitForQueueToBeEmpty(ctx, awsSession, queueName)
	if err != nil {
		zapctx.Fatal(ctx, "failed waiting for queue to be empty", zap.String("queue_name", queueName), zap.Error(err))
	}

	zapctx.Info(ctx, "sqs queue created", zap.String("queue_name", queueName))
}

func createS3Bucket(ctx context.Context, wg *sync.WaitGroup, awsSession *session.Session, bucketName string) {
	defer wg.Done()
	zapctx.Info(ctx, "creating s3 bucket", zap.String("bucket_name", bucketName))

	svc := s3.New(awsSession)
	_, err := svc.CreateBucketWithContext(ctx, &s3.CreateBucketInput{
		Bucket: aws.String(bucketName),
	})
	if err != nil && !strings.Contains(err.Error(), "BucketAlreadyExists") {
		zapctx.Fatal(ctx, "failed to create s3 bucket", zap.String("bucket_name", bucketName), zap.Error(err))
	}

	zapctx.Info(ctx, "s3 bucket created", zap.String("bucket_name", bucketName))
}

func setUpS3BucketNotifications(ctx context.Context, wg *sync.WaitGroup, awsSession *session.Session, bucketName string, queueName string) {
	defer wg.Done()
	zapctx.Info(ctx, "setting up notifications for s3 bucket", zap.String("bucket_name", bucketName))

	svc := s3.New(awsSession)
	_, err := svc.PutBucketNotificationConfigurationWithContext(ctx, &s3.PutBucketNotificationConfigurationInput{
		Bucket: aws.String(bucketName),
		NotificationConfiguration: &s3.NotificationConfiguration{
			QueueConfigurations: []*s3.QueueConfiguration{
				{
					Events: []*string{
						aws.String("s3:ObjectCreated:*"),
					},
					QueueArn: aws.String(queueName),
				},
			},
		},
	})
	if err != nil {
		zapctx.Fatal(ctx, "failed to set up bucket notifications",
			zap.String("bucket_name", bucketName),
			zap.String("sqs_name", queueName),
			zap.Error(err),
		)
	}
	zapctx.Info(ctx, "notifications for s3 bucket ans sqs queue created",
		zap.String("bucket_name", bucketName),
		zap.String("sqs_name", queueName),
	)
}

// sendSqsMsg - this func may be useful to test the complete flow (SQS-> listener -> BankingCircle)
// sends a message to sqs that's running on localstack
// nolint
func sendSqsMsg(awsSession *session.Session, queueName, queueURL, msgBody, deduplicationID string) error {
	svc := sqs.New(awsSession)

	msgInput := &sqs.SendMessageInput{
		DelaySeconds:            nil,
		MessageAttributes:       nil,
		MessageBody:             &msgBody,
		MessageDeduplicationId:  &deduplicationID,
		MessageGroupId:          nil,
		MessageSystemAttributes: nil,
		QueueUrl:                &queueURL,
	}

	_, err := svc.SendMessage(msgInput)
	return err
}

// Left this for documentation / history purpose
// nolint
func bankingCircleCertPrivateKey() string {
	return `-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----`
}

// Left this for documentation / history purpose
// nolint
func bankingCircleCertPublicKey() string {
	return `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`
}
